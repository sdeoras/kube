...
func (cdr *coder) Create(ctx context.Context) context.Context {
	...
	go func(input context.Context, done context.CancelFunc) {
		...
		case <-input.Done():
			var err error
			if cdr.config.ClusterRoleBinding, err = cdr.clientset.RbacV1beta1().ClusterRoleBindings().Create(cdr.config.ClusterRoleBinding); err != nil {
				log.Error(err)
				cdr.err <- err
			} else {
...
func (cdr *coder) Delete(ctx context.Context) context.Context {
	...
	go func(parent context.Context, f context.CancelFunc) {
		...
		case <-parent.Done():
			...
			if err := cdr.clientset.RbacV1beta1().ClusterRoleBindings().Delete(cdr.config.ClusterRoleBinding.Name, options); err != nil {
				log.Error(err)
				cdr.err <- err
				return
			} else {
