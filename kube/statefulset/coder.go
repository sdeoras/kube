...
func (cdr *coder) Create(ctx context.Context) context.Context {
	...
	go func(input context.Context, done context.CancelFunc) {
		...
		case <-input.Done():
			var err error
			if cdr.config.StatefulSet, err = cdr.clientset.AppsV1beta1().StatefulSets(cdr.namespace).Create(cdr.config.StatefulSet); err != nil {
				log.Error(err)
				cdr.err <- err
			} else {
...
func (cdr *coder) Delete(ctx context.Context) context.Context {
	...
	go func(parent context.Context, f context.CancelFunc) {
		...
		case <-parent.Done():
			...
			if err := cdr.clientset.AppsV1beta1().StatefulSets(cdr.namespace).Delete(cdr.config.StatefulSet.Name, options); err != nil {
				log.Error(err)
				cdr.err <- err
				return
			} else {
