...
func (cdr *coder) Create(ctx context.Context) context.Context {
	...
	go func(input context.Context, done context.CancelFunc) {
		select {
		case <-input.Done():
			if _, err := cdr.clientset.CoreV1().Endpoints(cdr.namespace).Create(cdr.config.Endpoints); err != nil {
				log.Error(err)
				cdr.err <- err
				return
			} else {
...
func (cdr *coder) Delete(ctx context.Context) context.Context {
	...
	go func(parent context.Context, f context.CancelFunc) {
		...
		case <-parent.Done():
			...
			if err := cdr.clientset.CoreV1().Endpoints(cdr.namespace).Delete(cdr.config.Endpoints.Name, options); err != nil {
				log.Error(err)
				cdr.err <- err
				return
			} else {
