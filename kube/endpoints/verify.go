...
func (cdr *coder) verifyCreate() error {
	...
	for {
		obj, err := cdr.clientset.CoreV1().Pods(cdr.namespace).Get(cdr.config.Endpoints.Name, *opts)
		if err != nil {
			log.Error(err)
			return err
		}
...
func (cdr *coder) verifyDelete() error {
	...
	opts := new(meta_v1.GetOptions)
	for {
		_, err := cdr.clientset.CoreV1().Endpoints(cdr.namespace).Get(cdr.config.Endpoints.Name, *opts)
		if err != nil {
			log.Info(err)
			return nil
		} else {
