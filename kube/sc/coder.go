...
func (cdr *coder) Create(ctx context.Context) context.Context {
	...
	go func(input context.Context, done context.CancelFunc) {
		...
		case <-input.Done():
			var err error
			if cdr.config.StorageClass, err = cdr.clientset.StorageV1beta1().StorageClasses().Create(cdr.config.StorageClass); err != nil {
				log.Error(err)
				cdr.err <- err
			} else {
...
func (cdr *coder) Delete(ctx context.Context) context.Context {
	...
	go func(parent context.Context, f context.CancelFunc) {
		...
		case <-parent.Done():
			...
			if err := cdr.clientset.StorageV1beta1().StorageClasses().Delete(cdr.config.StorageClass.Name, options); err != nil {
				log.Error(err)
				cdr.err <- err
				return
			} else {
