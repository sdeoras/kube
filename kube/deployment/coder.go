...
func (cdr *coder) Create(ctx context.Context) context.Context {
	...
	go func(input context.Context, done context.CancelFunc) {
		select {
		case <-input.Done():
			if _, err := cdr.clientset.AppsV1().Deployments(cdr.namespace).Create(cdr.config.Deployment); err != nil {
				log.Error(err)
				cdr.err <- err
				return
			} else {
...
func (cdr *coder) Delete(ctx context.Context) context.Context {
	...
	go func(parent context.Context, f context.CancelFunc) {
		...
		case <-parent.Done():
			...
			if err := cdr.clientset.AppsV1().Deployments(cdr.namespace).Delete(cdr.config.Deployment.Name, options); err != nil {
				log.Error(err)
				cdr.err <- err
				return
			} else {
